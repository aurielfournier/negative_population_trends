...
t20 <- do.call(rbind, dat$twenty_species) %>% mutate(spp_sim = paste0(variable,"_",sim))
t2 <- dat$real_data_highest %>% mutate(year=1:100) %>% gather("variable","value",-year)

ggplot() + 
        geom_point(data=t20, aes(x=year, y=value)) + 
        geom_point(data=t2, aes(x=year, y=value), col="grey")
