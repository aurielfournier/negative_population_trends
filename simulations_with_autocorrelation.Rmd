...
A. Not sure why you would need to subset out the simulations with
significant trends... or maybe I'm  just thinking about the objective
in a different way. I was thinking the outcome would be two types of
results: 
1. Mean and 95% of trends in the survey simulations compared
to mean and 95% of trends in all populations [or total species
abundance added up so that larger populations contribute more].  

2.
Proportion of populations with significant (declines) among the
surveyed populations, compared to proportion among all populations.
...
dat <- manysimulations(freq=bursts_of_sampling, sims=10)
...
valuessig <- values %>% filter(pvalue<=0.05 & beta < 0) %>% mutate(type="realtrend_significant")
svaluessig <- svalues %>% filter(pvalue<=0.05 & beta < 0) %>% mutate(type="surveyeddata_significant")

nrow(svaluessig)/nrow(svalues) # proportion of the sampled population that has significant negative trend
nrow(valuessig)/nrow(values) # proportion of the real population that has signficaint negative trend
...
B. One plot that might be instructive: randomly sample (use set.seed
so you can get these back again) 5 simulations and plot the trends in
the surveyed populations in black overlaid on the trends in all
populations (background in gray), together with the mean and 95%
interval for all 20 populations.

```{r}

dat <- manysimulations(sims=6)

t20 <- do.call(rbind, dat$twenty_species) %>% 
            mutate(sim=rep(c(1,2,3,4,5,6), each=2000)) 

dd20 <- lm(data=t20, value ~ year)

t20$fitted_value <- dd20$fitted.values

t20mean <- t20 %>%
            group_by(year) %>%
            summarize(mean=mean(fitted_value),
                      standD=sd(value),
                   sample_size=n(),
                   CI=qnorm(0.975)*standD/sqrt(sample_size))


t2 <- dat$real_data_highest %>% gather("variable","value") %>%  mutate(year=rep(c(1:100),times=12), sim=rep(c(1,2,3,4,5,6), each=200)) 


dd2 <- lm(data=t2, value ~ year)

t2$fitted_value <- dd2$fitted.values
...
ggplot()+
  geom_point(data=t20, col="grey",aes(x=year, y=value, group=variable))+
  geom_ribbon(data=t20mean, aes(x=year, ymin=mean-CI, ymax=mean+CI), alpha=0.7)+
  geom_line(data=t20mean, aes(x=year, y=mean))+
  geom_point(data=t2, aes(x=year, y=value))+
  stat_smooth(data=t2, aes(x=year, y=value, group=variable), method="lm")+
  theme_krementz()+
  facet_wrap(~sim)
...
dat <- manysimulations(sims=10000)

mean_survey <- mean(dat$real_values_highest$beta)
mean_pop <- mean(dat$real_values_all_pops$beta)
CI_survey <- quantile(dat$real_values_highest$beta, probs=c(0.05, 0.95))
CI_pop <- quantile(dat$real_values_all_pops$beta, probs=c(0.05, 0.95))

datdat <- data.frame(rbind(c(mean_survey, CI_survey),c(mean_pop, CI_pop)))
datdat$type <- c("survey","all_pops")
colnames(datdat) <- c("mean","lower","upper","type")
