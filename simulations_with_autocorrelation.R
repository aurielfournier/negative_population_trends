...
manysimulations <- function(sims=1000, freq=NA){
  ...
  real_values_highest <- data.frame(beta=rep(NA,sims*2),pvalue=rep(NA,sims*2),rsquared=rep(NA,sims*2))
  real_values_random <- data.frame(beta=rep(NA,sims*2),pvalue=rep(NA,sims*2),rsquared=rep(NA,sims*2))
  sampled_values_highest <- data.frame(beta=rep(NA,sims*2),pvalue=rep(NA,sims*2),rsquared=rep(NA,sims*2))
  sampled_values_random <- data.frame(beta=rep(NA,sims*2),pvalue=rep(NA,sims*2),rsquared=rep(NA,sims*2))
  ...
  # DO NOT FREAK OUT, EVERYTHING IS FINE
  # It just means you fed it a string of years to subset by and it is only checking to see if the first one is an NA. Which is ok. 
...
values <- dat$real_values_highest
svalues <- dat$real_values_highest
...
values <- dat$real_values_random
svalues <- dat$real_values_random
...
dat <- manysimulations(freq=bursts_of_sampling, sims=10000)
