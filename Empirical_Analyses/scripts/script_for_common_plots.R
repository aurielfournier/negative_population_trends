...
#determine highest abundance plots in first 1 year (hence the +1 in the filter() statement)
...
print(ranking_common_plots$count)
...
#
 if (first5=='no'){
   single_species_common <- single_species_common %>%
     filter(year > (single_species_common$year[which(is.na(single_species$count)==F)[1]] + 4))
 }
#
