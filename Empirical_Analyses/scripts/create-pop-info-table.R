...
# Last edited on 16-Apr-2019

# This code has been modified from White 2019 Bioscience
...
# Set to true to take log(count) instead of count
LOGPOP=TRUE
...
source('../scripts/remove_NA_populations.R')
...
for (j in 1:nrow(pop_info)){
    ...
    pop = pop[1:35,] # Take first 35 years as opposed to the last 35 years of dta
    ...
    if (LOGPOP == TRUE){
      min_pop_value = min(pop$popvalue[pop$popvalue>0])
      pop$popvalue[pop$popvalue==0]=min_pop_value
      pop$popvalue = log(pop$popvalue)
...
##### SAVE OUTPUT HERE ######
if (LOGPOP == FALSE){
  save(pop_info,long_dat,long_time_names,file = '../data/cleaned_timeseries_database.Rdata')
  ...
  save(pop_info,long_dat,long_time_names,file = '../data/cleaned_timeseries_database_with_min_time_linear_regression.Rdata')

}else{
  save(pop_info,long_dat,long_time_names,file = '../data/cleaned_timeseries_database_log_pop.Rdata')
  ...
  save(pop_info,long_dat,long_time_names,file = '../data/cleaned_timeseries_database_with_min_time_linear_regression_log_pop.Rdata')
}
