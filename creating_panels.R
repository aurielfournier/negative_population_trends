...
realtrend <- function(time_series, spp.=spp){
...
manysimulations <- function(sims=1000, freq=NA, spp=20, CV=0.2){
  ...
  for(i in seq(1,(sims*2),by=2)){
    dd <- onesimulation(freq=freq, spp.=spp, CV.=CV)
...
bar1 <- manysimulations(freq=1, spp=20, sims=10000)
b1 <- bar1$real_values_all_pops 
b1$bar <- "bar1"
bar2 <- manysimulations(freq=1, spp=20, sims=10000)
b2 <- bar2$real_values_random
b2$bar <- "bar2"
bar3 <- manysimulations(freq=1, spp=20, sims=10000)
b3 <- bar3$real_values_highest
b3$bar <- "bar3"
bar4 <- manysimulations(freq=2, spp=20, sims=10000)
b4 <- bar4$real_values_highest
b4$bar <- "bar4"
bar5 <- manysimulations(freq=5, spp=20, sims=10000)
b5 <- bar5$real_values_highest
b5$bar <- "bar5"

bars <- rbind(b1, b2, b3, b4, b5)
save(bars, "~/negative_population_trends/panel1.Rdata")

panel1 <- ggplot()+
  geom_boxplot(data=bars, aes(x=bar, y=beta))

ggsave(panel1, "~/negative_population_trends/panel1.jpeg", height=10, width=10, units="cm", dpi=300)

bar1 <- manysimulations(freq=1, spp=5, sims=10000)
b1 <- bar1$real_values_all_pops
b1$bar <- "bar1"
bar2 <- manysimulations(freq=1, spp=5, sims=10000)
b3 <- bar2$real_values_random
b2$bar <- "bar2"
bar3 <- manysimulations(freq=1, spp=5, sims=10000)
b3 <- bar3$real_values_highest
b3$bar <- "bar3"
bar4 <- manysimulations(freq=2, spp=5, sims=10000)
b4 <- bar4$real_values_highest
b4$bar <- "bar4"
bar5 <- manysimulations(freq=5, spp=5, sims=10000)
b5 <- bar5$real_values_highest
b5$bar <- "bar5"

bars <- rbind(b1, b2, b3, b4, b5)
save(bars, "~/negative_population_trends/panel2.Rdata")

panel2 <- ggplot()+
  geom_boxplot(data=bars, aes(x=bar, y=beta))

ggsave(panel2, "~/negative_population_trends/panel2.jpeg", height=10, width=10, units="cm", dpi=300)


bar1 <- manysimulations(freq=1, spp=50, sims=10000)
b1 <- bar1$real_values_all_pops
b1$bar <- "bar1"
bar2 <- manysimulations(freq=1, spp=50, sims=10000)
b2 <- bar2$real_values_random
b2$bar <- "bar2"
bar3 <- manysimulations(freq=1, spp=50, sims=10000)
b3 <- bar3$real_values_highest
b3$bar <- "bar3"
bar4 <- manysimulations(freq=2, spp=50, sims=10000)
b4 <- bar4$real_values_highest
b4$bar <- "bar4"
bar5 <- manysimulations(freq=5, spp=50, sims=10000)
b5 <- bar5$real_values_highest
b5$bar <- "bar5"

bars <- rbind(b1, b2, b3, b4, b5)
save(bars, "~/negative_population_trends/panel3.Rdata")

panel3 <- ggplot()+
  geom_boxplot(data=bars, aes(x=bar, y=beta))

ggsave(panel3, "~/negative_population_trends/panel3.jpeg", height=10, width=10, units="cm", dpi=300)


bar1 <- manysimulations(freq=1, spp=50, sims=10000, CV=0.6)
b1 <- bar1$real_values_all_pops
b1$bar <- "bar1"
bar2 <- manysimulations(freq=1, spp=50, sims=10000, CV=0.6)
b2 <- bar2$real_values_random
b2$bar <- "bar2"
bar3 <- manysimulations(freq=1, spp=50, sims=10000, CV=0.6)
b3 <- bar3$real_values_highest
b3$bar <- "bar3"
bar4 <- manysimulations(freq=2, spp=50, sims=10000, CV=0.6)
b4 <- bar4$real_values_highest
b4$bar <- "bar4"
bar5 <- manysimulations(freq=5, spp=50, sims=10000, CV=0.6)
b5 <- bar5$real_values_highest
b5$bar <- "bar5"

bars <- rbind(b1, b2, b3, b4, b5)
save(bars, "~/negative_population_trends/panel4.Rdata")

panel4 <- ggplot()+
  geom_boxplot(data=bars, aes(x=bar, y=beta))

ggsave(panel4, "~/negative_population_trends/panel4.jpeg", height=10, width=10, units="cm", dpi=300)


bar1 <- manysimulations(freq=1, spp=50, sims=10000, CV=0.05)
b1 <- bar1$real_values_all_pops
b1$bar <- "bar1"
bar2 <- manysimulations(freq=1, spp=50, sims=10000, CV=0.05)
b2 <- bar2$real_values_random
b2$bar <- "bar2"
bar3 <- manysimulations(freq=1, spp=50, sims=10000, CV=0.05)
b3 <- bar3$real_values_highest
b3$bar <- "bar3"
bar4 <- manysimulations(freq=2, spp=50, sims=10000, CV=0.05)
b4 <- bar4$real_values_highest
b4$bar <- "bar4"
bar5 <- manysimulations(freq=5, spp=50, sims=10000, CV=0.05)
b5 <- bar5$real_values_highest
b5$bar <- "bar5"

bars <- rbind(b1, b2, b3, b4, b5)
save(bars, "~/negative_population_trends/panel5.Rdata")

panel5 <- ggplot()+
  geom_boxplot(data=bars, aes(x=bar, y=beta))


ggsave(panel5, "~/negative_population_trends/panel5.jpeg", height=10, width=10, units="cm", dpi=300)
