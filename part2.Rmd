...
generate_species <- function(nyears=100, mean_log=7,sigma=0.2, rho=0.5,CV.=0.2, spp.=20) {
  
  sim.years <- nyears+10 # because we are going to chop off 10 years at the end
  
  mu = rlnorm(n=spp., meanlog = mean_log, sdlog = sigma) # log normal generation of starting population size 

  time_series <- matrix(nrow=sim.years,ncol=spp.) # creating the empty matrix
  
  time_series[1,] <- mu 
  
  for (i in 2:(sim.years)) {
    time_series[i,] <- time_series[(i-1),]+time_series[(i-1),]*sigma
  }
  
  time_series <- time_series + log(mu) - 0.5*sigma^2
...
generate_species <- function(nyears=100, mu=1000,rho=0.5,CV.=0.2, spp.=20) {
  ...
  time_series_temp <- matrix(rlnorm(n=spp., meanlog = mean_log, sdlog = sigma),ncol=spp., nrow=sim.years)
